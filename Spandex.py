...
def calculate_no_of_bags(spandex_quantity, spandex_yield, spandex_percentage):
    try:
        no_of_bags = (spandex_quantity * 100 * 100) / (spandex_percentage * spandex_yield * 45.36)
        return no_of_bags
    except ZeroDivisionError:
        st.error("Spandex Yield or Spandex Percentage cannot be zero!")
        return None
...
spandex_yield = st.number_input("Spandex Yield %:", min_value=0.0, format="%.2f")
...
if st.button("Calculate No. of Bags"):
    spandex_percentage = calculate_spandex_percentage(yarn_count, spandex_denier, draft)
    if spandex_percentage is not None:
        no_of_bags = calculate_no_of_bags(spandex_quantity, spandex_yield, spandex_percentage)
